<!DOCTYPE html>
<html lang="en">

<body>

<header class="hero">
  <h1>Daleeli 🗺️</h1>
  <p class="tagline">A travel companion for Egypt that tells you the truth, not the brochure version.</p>
  <div class="badges">
    <span class="badge">Flutter</span>
    <span class="badge">Firebase</span>
    <span class="badge">Gemini API</span>
    <span class="badge">Clean Architecture</span>
    <span class="badge">BLoC / Cubit</span>
  </div>
</header>

<div class="wrap">

  <h2>📖 About</h2>
  <p>
    Daleeli is a travel discovery app for tourist places in Egypt. Instead of just showing
    polished photos and star ratings, every place gets an <strong>Honest Truth</strong> card —
    generated by AI from real visitor reviews — surfacing genuine strengths and practical
    warnings (like "expect baksheesh requests here" or "closes earlier than listed") instead of
    brochure copy.
  </p>

<h2>📸 Screenshots</h2>
<p>Here are some screenshots from the app:</p>

<div class="screens-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 20px; align-items: start;">
  
  <!-- عنصر Splash -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">🌅 Splash</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (15)" src="https://github.com/user-attachments/assets/5c855c35-cad3-4e18-8268-5279cb588360" />
  </div>
  
  <!-- عنصر Onboarding -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">👋 Onboarding</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (16)" src="https://github.com/user-attachments/assets/51b852c9-4c06-4877-8355-4d5be09f5648" />
  </div>

  <!-- عنصر Sign In 1 -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">🔑 Sign In 1</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (17)" src="https://github.com/user-attachments/assets/311e47e8-25c0-4692-9b37-48af4f4b36bd" />
  </div>

  <!-- عنصر Sign In 2 -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">🔑 Sign In 2</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (18)" src="https://github.com/user-attachments/assets/79f0e518-d6c3-46ef-976b-7072b8108ffd" />
  </div>

  <!-- عنصر Explore -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">🗺️ Explore</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (19)" src="https://github.com/user-attachments/assets/befeff55-79f8-4fae-baf8-9282c446c6dc" />
  </div>

  <!-- عنصر Place Details -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">📍 Place Details</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (20)" src="https://github.com/user-attachments/assets/a709042e-5461-40e8-a996-41b85ce01e6b" />
  </div>

  <!-- عنصر Honest Truth -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">✨ Honest Truth</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (21)" src="https://github.com/user-attachments/assets/72b20728-1079-4160-a3a8-a009632956c3" />
  </div>

  <!-- عنصر My Trips -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">🧳 My Trips</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (22)" src="https://github.com/user-attachments/assets/a4616db4-36c8-4192-9c3d-e880f9b52120" />
  </div>

  <!-- عنصر Profile -->
  <div class="screen-slot" style="border: 1px solid #ddd; padding: 12px; border-radius: 12px; text-align: center; background: #fafafa;">
    <span style="font-weight: bold; display: block; margin-bottom: 8px;">👤 Profile</span>
    <img width="100%" style="border-radius:10px; height: auto;" alt="screenshot-rocks (23)" src="https://github.com/user-attachments/assets/7af735df-01cb-4f71-a44d-fc8f9fe20adc" />
  </div>

</div>


  <h2>✨ Features</h2>

  <div class="feature-card">
    <h4>Explore Real Places <span class="ai-tag" style="background:var(--olive);">DATA</span></h4>
    <p>Browse curated tourist places as postcard-style cards, with search and filters by governorate and category.</p>
  </div>

  <div class="feature-card">
    <h4>Honest Truth <span class="ai-tag">AI</span></h4>
    <p>Gemini analyzes real visitor review text for each place and generates an honest summary plus a list of practical warnings — displayed like a handwritten note.</p>
  </div>

  <div class="feature-card">
    <h4>Suggested Itineraries <span class="ai-tag">AI</span></h4>
    <p>Gemini composes multi-day trip plans from the places already in the app's dataset, grouped by region and duration.</p>
  </div>

  <div class="feature-card">
    <h4>Share Your Experience</h4>
    <p>Users can post a photo and short comment about a place they've visited, shown in a "Recent Experiences" feed on Home.</p>
  </div>

  <div class="feature-card">
    <h4>My Trips</h4>
    <p>Save places and revisit AI-suggested itineraries in one place.</p>
  </div>

  <h2>🛠 Tech Stack</h2>
  <table>
    <tr><th>Layer</th><th>Package / Tool</th></tr>
    <tr><td>Framework</td><td>Flutter</td></tr>
    <tr><td>State management</td><td>flutter_bloc / bloc (Cubit)</td></tr>
    <tr><td>Backend</td><td>Firebase — cloud_firestore, firebase_auth, firebase_core</td></tr>
    <tr><td>AI</td><td>Gemini API</td></tr>
    <tr><td>Networking</td><td>dio, pretty_dio_logger</td></tr>
    <tr><td>Dependency injection</td><td>get_it</td></tr>
    <tr><td>Functional error handling</td><td>dartz</td></tr>
    <tr><td>Images</td><td>cached_network_image</td></tr>
    <tr><td>UI / UX polish</td><td>google_fonts, flutter_animate, flutter_spinkit, shimmer, flutter_screenutil</td></tr>
    <tr><td>Config</td><td>flutter_dotenv (.env)</td></tr>
    <tr><td>Value equality</td><td>equatable</td></tr>
  </table>

  <h2>🏗 Architecture</h2>
  <p>The project follows Clean Architecture, split into three layers:</p>
  <pre><code>lib/
  data/            # data sources, models, repository implementations
  domain/          # entities, repository interfaces, use cases
  presentation/    # screens, widgets, cubits</code></pre>

  <h2>🚀 Getting Started</h2>
  <h3>1. Clone the project</h3>
  <pre><code>git clone https://github.com/your-username/daleeli.git
cd daleeli</code></pre>

  <h3>2. Install dependencies</h3>
  <pre><code>flutter pub get</code></pre>

  <h3>3. Set up environment variables</h3>
  <p>Create a <code>.env</code> file in the project root with your Gemini API key:</p>
  <pre><code>GEMINI_API_KEY=your_key_here</code></pre>

  <h3>4. Set up Firebase</h3>
  <ul>
    <li>Create a Firebase project at <code>console.firebase.google.com</code></li>
    <li>Enable Authentication (Email + Google) and Firestore</li>
    <li>Add your <code>google-services.json</code> (Android) / <code>GoogleService-Info.plist</code> (iOS)</li>
  </ul>

  <h3>5. Run the app</h3>
  <pre><code>flutter run</code></pre>

  <h2>📂 Seed Data</h2>
  <p>
    Initial place data (name, photo, category, governorate, reviews) lives in
    to Firestore on first run.
  </p>

  <h2>👩‍💻 Author</h2>
  <p>Toka Ahmed Elsharkawy — Flutter Developer</p>

  <div class="footer-note">Built as part of an 8-day bootcamp project · September 2026</div>

</div>
</body>
</html>
